...
這是一個基於 Python 與 PyQt5 開發的 UR5 工業手臂控制系統，模擬夾娃娃機的操作邏輯。使用者可以透過圖形化介面 (GUI) 控制手臂在 2D 平面移動並執行抓取動作。
...
pip install PyQt5
```
*(註：本專案主要使用 Python 內建套件如 `socket`, `threading`, `time` 等，無需額外安裝過多第三方庫)*

### 3. 設定與執行
1.  修改 `control/state_machine.py` 中的 `UR5_IP` 與 `UR5_PORT` 以符合您的設備設定。
2.  執行主程式：
    ```bash
    python main.py
    ```
